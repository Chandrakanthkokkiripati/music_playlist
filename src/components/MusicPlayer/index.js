...
class MusicPlayer extends Component {
  ...
  deleteSongItem = id => {
    ...
    const deletedList = playList.filter(each => each.id !== id)
    this.setState({playList: deletedList})
  }

  renderPlayList = () => {
    const {playList, searchInput} = this.state
    const updatedPlayList = playList.filter(each =>
      ...
      <>
        {updatedPlayList.length !== 0 ? (
          <ul className="songs-list-container">
            {updatedPlayList.map(eachSong => (
              <MusicItem
                key={eachSong.id}
                deleteSongItem={this.deleteSongItem}
